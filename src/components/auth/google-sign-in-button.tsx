...
export function GoogleSignInButton() {
    ...
    try {
      ...
      if (!userDoc.exists()) {
        ...
        if (res.status === 429) {
          ...
          // Fall through to /dashboard so WalletSetupCard handles recovery.
        } else if (!res.ok) {
          const err = await res.json().catch(() => ({}));
          throw new Error(err.error || "Wallet registration failed");
        } else {
          toast.success("Welcome. Your wallet is being created.");
        }
      }
  ...
  return (
    <Button
      variant="outline"
      className="h-11 w-full border-border bg-transparent text-foreground hover:bg-secondary"
